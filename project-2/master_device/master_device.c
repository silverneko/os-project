...
int master_mmap(struct file *filp, struct vm_area_struct *vm);
...
static long master_ioctl(struct file *file, unsigned int ioctl_num, unsigned long ioctl_param)
{
	...
	switch(ioctl_num){
		...
		case master_IOCTL_MMAP:
			{
				//this will be called when the copy is done
				//ioctl_param is the data size

				//printk("data size = %d\n", ioctl_param);
				ksend(sockfd_cli, userdata, ioctl_param, 0);
			}
			break;
